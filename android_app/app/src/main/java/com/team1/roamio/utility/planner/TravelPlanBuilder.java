...
public class TravelPlanBuilder {
    ...
    public void FixPlan(String target, String result, TravelPlanData targetData, PlanBuildCallback callback) {
        // 백그라운드 스레드에서 작업을 실행
        executor.execute(() -> {
            try {
                String prompt = createPrompt(target, result, targetData);
    ...
    private String createPrompt(String fixTarge, String fixResult, TravelPlanData targetPlanData) throws JSONException {
        // ... (프롬프트 문자열 생성 로직) ...
        return "You are an expert travel planner that outputs ONLY JSON but do not use code block.\n" +
                "Do not include any explanations, comments, or natural language text outside the JSON.\n\n" +
                "---\n" +
                ...
                userCustomData.toString() + "\n\n" +
                "### Input (Existing Data):\n" +
                TravelPlanParser.parsePlanDataToJson(targetPlanData).toString() + "\n\n" +
                "### Input (User Requests):\n" +
                "- Fix Target: " + fixTarge + "\n" +
                "- Fix Result: " + fixResult + "\n\n" +
                "- Fix Target 대신 Fix Result나 그 근처 여행지를 방문하는 것으로 Existing Data를 수정해주세요." +
                "---\n" +
                "### Rules for Output:\n" +
                "1. Output format: Return ONLY a JSON object with these keys:\n" +
                "   - \"plan_summary\": 여행 전체를 요약한 한 문장 (한국어)\n" +
