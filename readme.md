...
5. google-services.json 및 필요한 인증 파일을 앱에 추가
6. 빌드 및 실행
   - Android Studio에서 에뮬레이터 또는 실기기로 실행


## 구성 파일 및 중요 포인트
- 주요 코드: app/src/main/java/ (인증, AI 연동, Drive CRUD, DTO 파서)  
- 리소스: app/src/main/res/ (레이아웃, 이미지, 문자열)  
- 환경값: local.properties, google-services.json

## 예시 플로우(개발자용)
- 사용자 입력 → RequestBuilder → Gemini 호출 → ResponseParser(DTO) → PlannerModel → ViewModel → UI  
- 저장: PlannerModel → JSON 직렬화 → Drive AppData 업로드

## 기여 가이드
1. 이슈 확인 또는 신규 이슈 등록
2. 포크 → feature/{name} 브랜치 생성
3. 작업 후 커밋(컨벤션: feat/fix/docs 등)
4. PR 생성 및 코드리뷰 후 병합

## 보안 및 개인정보
- 모든 개인 데이터는 사용자의 Google Drive(AppData)에 저장됩니다.  
- 서버에 민감데이터를 저장하지 않도록 설계되어 있으나, OAuth 토큰 취급은 안전하게 처리해야 합니다.


## 주요 기여자 (Team 1)
- 주민재 — Scrum Master & Full-stack (아키텍처, OAuth/Drive 연동, 실제 UI)  
- 이영욱 — Sound & Resource (효과음, BGM, 리소스 관리)  
